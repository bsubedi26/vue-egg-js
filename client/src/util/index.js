...
export function getDomain () {
  ...
  const apiUrl = process.env.NODE_ENV === 'development' ? config.dev.serverRoot : config.build.serverRoot
  return `${http}${apiUrl}`
  // return httpProtocol + (process.env.NODE_ENV === 'development' ? config.dev.serverRoot : config.build.serverRoot)
}
